  ...
  return {
    control: {
      create: jest
        ...
        .mockImplementation(({ data }) =>
          Promise.resolve({ ...data, id: `ctl_new_${Math.random()}` }),
        ),
      update: jest.fn(),
    ...
    task: {
      create: jest
        .fn()
        .mockImplementation(({ data }) =>
          Promise.resolve({ ...data, id: `tsk_new_${Math.random()}` }),
        ),
      update: jest.fn(),
      findMany: jest.fn().mockResolvedValue([]),
    },
    policy: {
      create: jest
        .fn()
        .mockImplementation(({ data }) =>
          Promise.resolve({ ...data, id: `pol_new_${Math.random()}` }),
        ),
      update: jest.fn(),
      findMany: jest.fn().mockResolvedValue([]),
    },
    policyVersion: {
      create: jest.fn().mockResolvedValue({ id: 'pv_new', version: 2 }),
    ...
    requirementMap: {
      create: jest
        .fn()
        .mockImplementation(({ data }) =>
          Promise.resolve({ ...data, id: `rm_new_${Math.random()}` }),
        ),
      update: jest.fn(),
      updateMany: jest.fn(),
      findMany: jest.fn().mockResolvedValue([]),
    ...
    frameworkSyncOperation: {
      create: jest.fn().mockResolvedValue({ id: 'fso_new' }),
    },
    ...
    controlDocumentType: {
      findUnique: jest.fn().mockResolvedValue(null),
      create: jest.fn().mockResolvedValue({ id: 'cdt_new' }),
      delete: jest.fn(),
    },
    ...
    frameworkControlDocumentTypeLink: {
      findUnique: jest.fn().mockResolvedValue(null),
      create: jest.fn().mockResolvedValue({ id: 'fdl_new' }),
      createMany: jest.fn().mockResolvedValue({ count: 0 }),
      deleteMany: jest.fn().mockResolvedValue({ count: 0 }),
    },
...
describe('applySync', () => {
  it('creates new control instance when added in target manifest (no existing row in org)', async () => {
    ...
    expect(tx.control.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          organizationId: 'org_1',
          name: 'New Control',
          description: 'd',
          controlTemplateId: 'ct_new',
        }),
      }),
    );
    expect(result.syncOperationId).toBe('fso_new');
    expect(tx.frameworkInstance.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'frm_1' },
  ...
  it('archives control when removed from target manifest and no other framework references it', async () => {
    ...
    expect(tx.control.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 'ctl_stale' },
  ...
  it('does NOT archive control when another framework references it', async () => {
    ...
    expect(tx.control.update).not.toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ archivedAt: expect.any(Date) }),
      }),
    );
  ...
  it('creates a draft PolicyVersion for published policies when template content changes', async () => {
    ...
    expect(tx.policyVersion.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          policyId: 'pol_1',
          content: { set: [{ new: true }] },
        }),
      }),
    );
    expect(tx.policy.update).not.toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('creates RequirementMap edge when link appears in target manifest', async () => {
    ...
    await applySync(tx, {
      ...
      currentVersion: {
        ...
        manifest: manifest({
          requirements: [
            { id: 'rq_1', identifier: 'CC1', name: 'X', description: null },
          ],
          controls: [
            {
              id: 'ct_1',
    ...
    expect(tx.requirementMap.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          controlId: 'ctl_1',
          requirementId: 'rq_1',
  ...
  it('archives RequirementMap edge when link disappears', async () => {
    ...
    expect(tx.requirementMap.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('unarchives an existing archived RequirementMap row instead of creating a duplicate', async () => {
    ...
    expect(tx.requirementMap.update).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('inserts missing _ControlToPolicy edge when v1 and v2 both claim it but customer has no row (drift)', async () => {
    ...
    const calls = tx.$executeRaw.mock.calls.map((c: unknown[]) =>
      String(c[0]?.[0] ?? ''),
    );
    const cpInsertCalled = calls.some((s: string) =>
      s.includes('INSERT INTO "_ControlToPolicy"'),
    );
  ...
  it('creates missing task row before reconciling _ControlToTask drift when v1 and v2 both claim it', async () => {
    ...
    const ctInsertCalled = calls.some((s: string) =>
      s.includes('INSERT INTO "_ControlToTask"'),
    );
  ...
  it('creates missing ControlDocumentType row when v1 and v2 both claim it but customer has no row (drift)', async () => {
    ...
    expect(tx.controlDocumentType.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          controlId: 'ctl_1',
  ...
  it('does NOT re-insert a _ControlToPolicy edge that already exists in the customer DB (no double-undo)', async () => {
    ...
    // still wants.
    const cpInserts = tx.$executeRaw.mock.calls
      .map((c: unknown[]) => String(c[0]?.[0] ?? ''))
      .filter((s: string) => s.includes('INSERT INTO "_ControlToPolicy"'));
    expect(cpInserts).toHaveLength(0);

    const undoPayload =
      tx.frameworkSyncOperation.create.mock.calls[0][0].data.undoPayload;
