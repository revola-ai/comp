...
`backfillFrameworkScopedLinks` in `seed.ts` is changed so that, for a pinned instance, the target link set is computed from the pinned manifest: for each manifest control, resolve the organization's control, policy and task rows by `(organizationId, templateId)` (every matching row is included when duplicates exist; missing rows are skipped and reported), and map `documentTypes` through the same enum conversion the migration uses. The instance's `FrameworkControlPolicyLink`, `FrameworkControlTaskLink` and `FrameworkControlDocumentTypeLink` rows are then made equal to the target set: missing rows inserted, rows outside the set deleted. Unpinned instances keep today's insert-only organization-level behaviour.
