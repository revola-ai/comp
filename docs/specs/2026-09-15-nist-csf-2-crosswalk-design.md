...
Status: revision 4, for review.
Branch: `revola/csf-2.0` (from `revola/self-host`).
Date: 2026-09-15.
Revision 1 was reviewed adversarially (Codex, read-only against the sources below); revision 2 incorporated the 17 findings and was re-reviewed; revision 3 addressed the items still open after that pass; a third pass verified transcription and sourcing as passing and left code/schema and marker-consistency items, addressed in revision 4. All rounds are summarized in section 11.
...
Each row names the parent control(s), quotes the control description, policy clause or task description that supports the choice, names the evidence task, and is marked "Judgment mapping." whenever no quoted span in the row comes from a mapped control's own description (that is, whenever a linked policy, a task, or procedure content carries the outcome). The generator enforces this rule; 47 of 106 rows carry the marker.
...
Today's backfill manifest already contains `documentTypes`; the fields it lacks relative to the normal builder are `requirementFamily`, `sortOrder` and `controlFamily`.
...
1. For every framework, insert scoped policy, task and document-type link rows derived from the global relations for the controls mapped to that framework's requirements, with `ON CONFLICT DO NOTHING` on the composite unique keys. This is the migration's fallback query with its "frameworks without a published version" filter removed. Fork decision: for every framework except CSF the seed treats the global union as the baseline and is insert-only; it never deletes a scoped row. The framework editor's services (`apps/api/src/framework-editor/control-template/control-template.service.ts`, `framework.service.ts`, import) create and delete scoped rows too, so an editor deletion of a global-derived link on a non-CSF framework is re-added by the next seed run. That is a known limitation of this fork: non-CSF scoped links should be curated by editing the global relations (which the seed respects), not by deleting scoped rows in the editor. Extending crosswalk-style authority to other frameworks is the way to remove the limitation later.
...
`backfillFrameworkScopedLinks` in `seed.ts` is changed so that, for a pinned instance, the target link set is computed from the pinned manifest: for each manifest control, resolve the organization's control, policy and task rows by `(organizationId, templateId)` (every matching row is included when duplicates exist; missing rows are skipped and reported), and map `documentTypes` through the same enum conversion the migration uses. Missing rows are inserted into the instance's `FrameworkControlPolicyLink`, `FrameworkControlTaskLink` and `FrameworkControlDocumentTypeLink` tables; no row is ever deleted. User-authored instance links are preserved. A template link removed by a user is re-added on the next seed run, the same limitation as the unpinned fallback and as the scoped-link baseline in step 1. Unpinned instances keep today's insert-only organization-level behaviour.
...
10. Reconciliation: scoped links belong to (framework, control), not (requirement, control). Case A: remove a control from one of several CSF requirements that use it; after seeding, that requirement-control row is gone and the control's CSF scoped rows remain. Case B: remove the only CSF reference to a control; after seeding, its requirement-control row and its CSF scoped rows are gone. In both cases every other framework's requirement-control and scoped-link sets are unchanged, and a second seed run changes nothing.
11. Scoped-link population is idempotent: running the seed twice yields identical scoped-link row sets for every framework.
12. Instance isolation: for an organization with SOC 2 and CSF instances, after adding CSF and re-running the seed, the SOC 2 instance's `FrameworkControlTaskLink` and `FrameworkControlPolicyLink` sets contain every link derived from its pinned manifest, contain none of the CSF-only task ids, and keep a user-authored link to a custom policy.
...
- SOC 2: 63 requirements, 35 controls, 25 policies, 26 tasks unchanged; its instance-level link edge sets (63, 76, 9 rows) equal before and after adding CSF and after a further seed run.
...
5. 47 rows are marked "Judgment mapping." under the enforced rule (no quoted span from a mapped control's own description); the rationale says what carries the outcome (a policy clause or a task) or that it is procedure content. Mapping all 106 outcomes is not the same as evidencing all 106 outcomes: 59 rows are stated by a control description, 47 rest on policy clauses, tasks or procedures.
...
| **GV.RM-02** Risk appetite and risk tolerance statements are established, communicated, and maintained | PM-09 | Risk Appetite & Tolerance (new) | No existing control or task produces appetite and tolerance statements. Parent: new Risk Appetite & Tolerance control ("Establish, communicate and maintain risk appetite and risk tolerance statements"). Evidence: Risk Appetite Statement (new). |
...
| **GV.RM-05** Lines of communication across the organization are established for cybersecurity risks, including risks from suppliers and other third parties | PM-09, PM-30 | Risk Management `b97c26af`; Supplier & Third-Party Security `ffb17d27` | Parent: Risk Management and Supplier & Third-Party Security. Neither description states communication lines; the Risk Register & Treatment Plan task (new) records "how risks are communicated across the organization and with suppliers". Judgment mapping. |
| **GV.RM-06** A standardized method for calculating, documenting, categorizing, and prioritizing cybersecurity risks is established and communicated | PM-09, PM-18, PM-28, PM-30, RA-03 | Risk Management `443cbf5b` | Parent: the ISO-variant Risk Management control, described as a "Formal risk identification, assessment, treatment, and monitoring process, including risk register and treatment plans". A formal process is the basis for a standardized method; the specific calculation, categorization and communication rules are procedure content. Evidence: Risk Register & Treatment Plan (new). Judgment mapping. |
...
| **GV.PO-02** Policy for managing cybersecurity risks is reviewed, updated, communicated, and enforced to reflect changes in requirements, threats, technology, and organizational mission | AC-01, AT-01, AU-01, CA-01, CM-01, CP-01, IA-01, IR-01, MA-01, MP-01, PE-01, PL-01, PM-01, PS-01, PT-01, RA-01, SA-01, SC-01, SI-01, SR-01 | Policy Compliance `624c59c1` | Parent: Policy Compliance ("Ensure Compliance with Security Policies and Standards"); the review and update cadence is supplied by its linked "Policy Management & Exception Handling" policy, not by the control description. Judgment mapping. |
...
| **GV.OV-02** The cybersecurity risk management strategy is reviewed and adjusted to ensure coverage of organizational requirements and risks | PM-09, PM-19, PM-30, PM-31, RA-07, SR-06 | Internal Audit & Management Review `29676acb`; Performance Monitoring & Measurement `bf7686cf` | Parent: Internal Audit & Management Review ("facilitates periodic management review of ISMS effectiveness"), where strategy adjustments would be decided; Performance Monitoring & Measurement supplies the inputs. Adjusting the strategy itself is not stated by either description. Evidence: Management Review Minutes, Security Metrics Report (new). Judgment mapping. |
...
| **GV.SC-04** Suppliers are known and prioritized by criticality | RA-09, SA-09, SR-06 | Supply Chain Risk Management Program (new); Supplier & Third-Party Security `ffb17d27` | Parent: Supplier & Third-Party Security for the supplier register; the new program control for prioritization. Ranking by criticality is stated by the Supply Chain Risk Register task (new), "suppliers ranked by criticality", not by either control description. Judgment mapping. |
...
| **ID.AM-04** Inventories of services provided by suppliers are maintained | AC-20, SA-09, SR-02 | Supplier & Third-Party Security `ffb17d27`; Asset Inventory `2f9df060` | Parent: Supplier & Third-Party Security ("supply-chain risk for ICT services") and Asset Inventory ("Maintain an Inventory of Information Assets"). A specific inventory of supplier-provided services is not stated by either. Evidence: Supplier Evaluation Records, Infrastructure Inventory. Judgment mapping. |
...
| **ID.RA-04** Potential impacts and likelihoods of threats exploiting vulnerabilities are identified and recorded | PM-09, PM-11, RA-02, RA-03, RA-08, RA-09 | Risk Management `443cbf5b` | Parent: ISO-variant Risk Management ("Formal risk identification, assessment, treatment, and monitoring process"). Impact and likelihood recording is stated by the Risk Register & Treatment Plan task (new), "likelihood and impact". Judgment mapping. |
| **ID.RA-05** Threats, vulnerabilities, likelihoods, and impacts are used to understand inherent risk and inform risk response prioritization | PM-16, RA-02, RA-03, RA-07 | Risk Management `443cbf5b`; Risk Management `b97c26af` | Parent: both Risk Management controls. Combining threats, vulnerabilities, likelihoods and impacts into prioritization is stated by the Risk Register & Treatment Plan task (new), "resulting prioritization", not by the control descriptions. Judgment mapping. |
...
| **ID.RA-07** Changes and exceptions are managed, assessed for risk impact, recorded, and tracked | CA-07, CM-03, CM-04 | Change management `7fa58e13`; Policy Compliance `624c59c1` | Parent: Change management ("Apply Change Management for Information Systems") for changes; exceptions are handled under Policy Compliance through its linked "Policy Management & Exception Handling" policy. Risk assessment of each change is procedure content. Judgment mapping. |
...
| **PR.AA-02** Identities are proofed and bound to credentials based on the context of interactions | IA-12 | Access Rights `40bf3583`; Personnel Security `4e63220f` | Parent: Access Rights ("Manage User Access Rights Lifecycle") and Personnel Security. Proofing at onboarding is evidenced by Employee Verification; binding of identities to credentials across contexts is not stated by either description. Judgment mapping. |
| **PR.AA-03** Users, services, and hardware are authenticated | AC-07, AC-12, IA-02, IA-03, IA-05, IA-07, IA-08, IA-09, IA-10, IA-11 | Credential Management `e558a6f5` | Parent: Credential Management ("Protect Authentication Information"). Evidence: 2FA and Secure Secrets cover user and service authentication; hardware authentication is not evidenced. Judgment mapping. |
...
| **PR.DS-11** Backups of data are created, protected, maintained, and tested | CP-06, CP-09 | Disaster Recovery Planning `ae323c9f` | Parent: Disaster Recovery Planning ("Test DR; meet RTO/RPO"), whose Backup policy governs backups. Evidence: Backup Restoration Test and Backup logs (both CSF-scoped). Backup creation and protection are policy and task content, not the control description. Judgment mapping. |
...
| **PR.PS-03** Hardware is maintained, replaced, and removed commensurate with risk | CM-07(09), SA-10(03), SC-03(01), SC-39(01), SC-49, SC-51 | Technology Asset Lifecycle (new) | Parent: new Technology Asset Lifecycle control ("acquisition, maintenance, replacement and removal"). Evidence: Asset Disposal & Sanitization Log (new), Device List. |
...
| **DE.CM-02** The physical environment is monitored to find potentially adverse events | CA-07, PE-03, PE-06, PE-20 | Physical Access Control `676d93db` | Parent: Physical Access Control ("Control Physical Entry to Facilities"). Monitoring, as distinct from control, is evidenced by the linked tasks Office Access & Door Monitoring and Visitor Control. Judgment mapping. |
...
| **DE.CM-09** Computing hardware and software, runtime environments, and their data are monitored to find potentially adverse events | AC-04, AC-09, AU-12, CA-07, CM-03, CM-06, CM-10, CM-11, SC-34, SC-35, SI-04, SI-07 | Security Monitoring & Detection `93773c1d`; Endpoint Protection `669076cc`; Security Logging `b4c2c4d8` | Parent: Security Monitoring & Detection ("Central SIEM with alerting"), Endpoint Protection ("Protect Systems Against Malware") and Security Logging ("Enable Logging of Security Events"). Together they cover software, endpoint and event monitoring; comprehensive runtime and data monitoring is not stated. Judgment mapping. |
...
| Subcategory (NIST CSF 2.0 text) | NIST informative refs (SP 800-53 Rev 5, union of 5.1.1 and 5.2.0 entries) | Comp controls | Rationale |
|---|---|---|---|
| **DE.AE-02** Potentially adverse events are analyzed to better understand associated activities | AU-06, CA-07, IR-04, SI-04 | Security Monitoring & Detection `93773c1d`; Security Incident Management `03d6bee8` | Parent: Security Monitoring & Detection ("Central SIEM with alerting") for analysis; Security Incident Management for escalation. Characterization of events is procedure content. Evidence: Monitoring & Alerting, Incident Response. Judgment mapping. |
...
| **RS.MA-05** The criteria for initiating incident recovery are applied | IR-04, IR-08 | Security Incident Management `03d6bee8`; Disaster Recovery Planning `ae323c9f` | Parent: Security Incident Management and Disaster Recovery Planning, whose linked policy requires "Define DR trigger thresholds; an Incident Commander authorises activation and coordinates restoration". The criteria are policy content, not control description. Judgment mapping. |
...
| Subcategory (NIST CSF 2.0 text) | NIST informative refs (SP 800-53 Rev 5, union of 5.1.1 and 5.2.0 entries) | Comp controls | Rationale |
|---|---|---|---|
| **RS.MI-01** Incidents are contained | IR-04 | Security Incident Management `03d6bee8` | Parent: Security Incident Management; its linked policy section "Containment & Eradication" requires "Isolate affected systems within 30 minutes of confirmation". Containment is policy content, not control description. Judgment mapping. |
| **RS.MI-02** Incidents are eradicated | IR-04 | Security Incident Management `03d6bee8`; Vulnerability Management `72aba39a` | Parent: Security Incident Management, whose linked policy has a "Containment & Eradication" section; removing the exploited weakness is Vulnerability Management ("Manage both software and code vulnerabilities"). Eradication is policy content. Judgment mapping. |
...
| Subcategory (NIST CSF 2.0 text) | NIST informative refs (SP 800-53 Rev 5, union of 5.1.1 and 5.2.0 entries) | Comp controls | Rationale |
|---|---|---|---|
| **RC.RP-01** The recovery portion of the incident response plan is executed once initiated from the incident response process | CP-10, IR-04, IR-08 | Disaster Recovery Planning `ae323c9f`; Security Incident Management `03d6bee8` | Parent: Disaster Recovery Planning ("Test DR; meet RTO/RPO") executed when incident procedures invoke recovery. Testing and planning evidence (Planning, Backup Restoration Test) does not itself show execution during an actual incident. Judgment mapping. |
...
| **RC.CO-04** Public updates on incident recovery are shared using approved methods and messaging | CP-02, IR-04 | Incident & Recovery Communications (new) | Parent: new communications control, whose description states "which approved methods and messaging are used for public updates". Evidence: Stakeholder Communication Plan (new). |
...
- 47 of 106 rows are judgment mappings where the outcome is carried by a task or policy clause or is procedure content. The rows themselves say which. Auditors assessing CSF alignment will need the procedure content in those cases.
...
- Moving the manifest builder into `packages/db` and changing the two backfills are divergences from upstream that merges must carry. The builder move changes its error contract and client import (5.3); the instance backfill change alters behaviour only for pinned instances.
...
- Every quoted span in every rationale is checked by the generator as an exact substring of a text belonging to a mapped control or its linked policies and tasks, and the judgment marker is enforced by the generator.
...
Round 3 (verification of revision 3: transcription and sourcing passed; code/schema and marker consistency incomplete), addressed in revision 4:

- Builder move: Nest and alias dependencies and the error contract stated; deterministic array ordering specified; `documentTypes` removed from the list of fields the backfill lacks.
- Instance-level backfill: exact-set semantics (insert missing, delete extra), duplicate and missing row handling, enum conversion; rollout claim about SOC 2's links replaced by a recorded before/after edge-set check.
- Final review: exact-set semantics replaced by insert-only from the pinned manifest because the application stores user-authored links (custom policies and controls, links added from the framework control page) in the same tables and the reconcile deleted them on reseed; spec test 12 is now an automated scratch-database test.
- Non-CSF scoped-link authority: the editor services are acknowledged as writers; the seed's insert-only behaviour and its consequence are documented as a fork limitation.
- Test 10 corrected for (framework, control) scoping of links.
- Judgment marker made a generator-enforced rule; 47 rows now carry it (GV.RM-05, GV.SC-04, GV.PO-02, RS.MA-05, RS.MI-01, RS.MI-02, PR.DS-11 and others); GV.RM-06, PR.AA-02, PR.AA-03, DE.CM-09 and RC.RP-01 bounded.
